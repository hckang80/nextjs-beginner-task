...
export function ChannelContainer() {
  ...
  const handleSelectAll = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const result = (list: ChannelItem[]) => list.map((item) => ({ ...item, checked }));

    setDataGroups(result);
  };

  const isGroupSelected = (type: string) =>
    dataGroups.filter((item) => item.type === type).every((item) => item.checked);

  const handleSelectGroup = (type: string) => (event: React.ChangeEvent<HTMLInputElement>) => {
    const { checked } = event.target;
    const result = (list: ChannelItem[]) =>
